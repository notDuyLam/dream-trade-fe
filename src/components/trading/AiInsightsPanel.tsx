  ...
  return (
    <div className="space-y-4">
      {props.insights.map(insight => (
        <article
          key={insight.id}
          className="rounded-2xl border border-slate-800 bg-slate-900/40 p-5 text-sm text-slate-200 shadow-lg shadow-black/20"
        >
          ...
          <div className="mt-3 flex items-baseline justify-between">
            <p className={`text-lg font-semibold ${directionAccent[insight.direction]}`}>
              {directionCopy[insight.direction]}
            </p>
