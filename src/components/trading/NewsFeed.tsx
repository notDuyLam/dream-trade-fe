  ...
  return (
    <div className="space-y-4">
      {props.articles.map(article => (
        <article
          key={article.id}
          className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4 text-sm text-slate-200 shadow-lg shadow-black/20"
        >
          ...
          <p className="mt-1 text-slate-400">{article.excerpt}</p>

          <div className="mt-3 flex flex-wrap gap-2 text-xs font-semibold">
            <span className={`rounded-full px-3 py-1 ${sentimentClasses[article.sentiment]}`}>
