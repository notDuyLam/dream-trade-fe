...
export default async function DashboardPage(props: DashboardPageProps) {
  ...
  const DEFAULT_SYMBOL: TradingSymbol
    = (searchParams.symbol as TradingSymbol) || 'BTCUSDT';
  const DEFAULT_TIMEFRAME: Timeframe = '1h';

  // Validate symbol using CoinService
  const validSymbols = CoinService.getAllSymbols();
  const symbol = CoinService.isValidSymbol(DEFAULT_SYMBOL)
    ? DEFAULT_SYMBOL
    : (validSymbols[0] ?? 'BTCUSDT');
